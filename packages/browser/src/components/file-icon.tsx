...
const FolderIcon = forwardRef<SVGSVGElement, FolderIconProps>(
  ({ className, size = 24 }, ref) => {
    return (
      <svg
        ...
        {...baseProps(
          size,
          cn("text-amber-600 dark:text-amber-400", className),
        )}
        aria-hidden="true"
      >
        <path
          d="M3 6.5A2.5 2.5 0 0 1 5.5 4h4l2 2H18.5A2.5 2.5 0 0 1 21 8.5v8A2.5 2.5 0 0 1 18.5 19h-13A2.5 2.5 0 0 1 3 16.5v-10Z"
          fill="currentColor"
        />
      </svg>
    );
...
const FileIcon = forwardRef<SVGSVGElement, FileIconProps>(
  ({ type, className, size = 24 }, ref) => {
    ...
    const iconClass = cn(colors[type], className);

    if (type === "image") {
      return (
        <svg ref={ref} {...baseProps(size, iconClass)} aria-hidden="true">
          <rect
            x="3"
            y="4"
            width="18"
    ...
    if (type === "audio") {
      return (
        <svg ref={ref} {...baseProps(size, iconClass)} aria-hidden="true">
          <path
            d="M13 5v10.5a2.5 2.5 0 1 1-1.5-2.29V8h7V5h-5.5Z"
            fill="currentColor"
          />
        </svg>
      );
