...
export function useFilePreview(
  ...
  const openPreview = useCallback(async (item: BrowserItem) => {
  ...
  const navigatePreview = useCallback(async (direction: "prev" | "next") => {
    const state = store.getState();
    const files = state.items.filter((item): item is BrowserFile => item.kind === "file");
  ...
  const previewType = previewItem && previewItem.kind === "file"
